...
def randomizer(): # Function of randomizer checkbox
    print("[log] Randomizer:", isRandomizer.get())

def start_service(username):
    # connection(username)
    print("Click")

def enable_log_panel():
    print("[log] Log panel:", isLogPanelEnabled.get())

    if (isLogPanelEnabled.get() == "on"):
        logPanel.grid(row=4, column=0, padx=10, sticky="ew", columnspan=2)
    else:
        logPanel.grid_remove()
...
btnLogPanel = tk.CTkSwitch(root, text="Enable Log Panel", command=enable_log_panel,
                                    variable=isLogPanelEnabled, onvalue="on", offvalue="off")
btnLogPanel.grid(row=3, column=0, padx=10, pady=10, sticky="w")

logPanel = tk.CTkTextbox(root)
logPanel.insert("1.0", "log ")  # insert at line 0 character 0
text = logPanel.get("0.0", "end")  # get text from line 0 character 0 till the end
logPanel.configure(state="disabled")  # configure textbox to be read-only

    # jsonPanel = tk.CTkTextbox(root)
    # jsonPanel.insert("0.0", "service_disabled")  # insert at line 0 character 0
    # text = jsonPanel.get("0.0", "end")  # get text from line 0 character 0 till the end
    # # jsonPanel.grid(row=0, column=3, padx=10, pady=10, sticky="ns", rowspan=5) # Make this panel another window

    # Start services

btnStartServices = tk.CTkButton(root, text="Start Services", command=start_service(username))
